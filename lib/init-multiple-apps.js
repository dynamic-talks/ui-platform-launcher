...
import { createServerIocContainer } from 'ui-platform-core/dist/lib/ui-application/server.ioc-container';
...
export default function({ appsPath }) {
  ...
  appSettings.forEach(({ rootPath }) => {
    app.use(express.static(path.join(rootPath, 'public')));
  });

  // initialize app pages
  appSettings.forEach(({ rootPath, name, version, params }) => {
    const baseConfigPath = path.join(rootPath, 'config', 'base.yaml');
    const { configPath, assetsManifestPath } = params;
    let absConfigPath;

    if (configPath) {
      absConfigPath = path.join(rootPath, configPath);
    } else {
